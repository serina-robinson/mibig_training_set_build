...
# Write to CSV file
# write.csv(aap, paste0("data/797_seqs_510_feats_for_supervised_scaled_20191104.csv"), row.names=rownames(aap), quote = F)

# Read in the data
rawdat <- read_csv("data/797_seqs_510_feats_for_supervised_scaled_20191104.csv")
#  <- data.frame(cbind(rawdat$X1), scale(rawdat[,2:ncol(rawdat)]), stringsAsFactors = F)

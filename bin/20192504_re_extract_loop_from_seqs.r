...
write.csv(extract_34_df, "output/655_uniprot_train_loop_extracted.csv", row.names = T, quote = F)
...
write.csv(extract_34_df_NRPS, "output/150_NRPS_train_loop_extracted.csv", quote = F, row.names = T)

# Combine the NRPS and the Uniprot
comb_df <- read_csv("output/655_uniprot_train_loop_extracted.csv") %>%
  column_to_rownames("X1")

comb_df2 <- read_csv("output/150_NRPS_train_loop_extracted.csv") %>%
  column_to_rownames("X1")

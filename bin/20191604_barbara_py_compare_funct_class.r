...
# Remove duplicates
grp_dedup <- grp_comb[!duplicated(grp_comb)]

# Convert to seq features
source("src/convert_seq_15aap.r")
res <- strsplit(as.character(grp_dedup), "")
nlist <- lapply(1:length(grp_dedup), function(x) convert_seq_15aap(as.character(res[[x]])))
...
rownames(extract_34_df) <- names(grp_dedup)
head(extract_34_df)
dim(extract_34_df)
# write.csv(extract_34_df, "data/1305_seqs_510_features_centered_scaled_dups_removed.csv", quote = F, row.names = T)

# Read in the data
rawdat <- read_csv("data/1305_seqs_510_features_centered_scaled_dups_removed.csv")
colnames(rawdat)[1] <- "nms"
head(rawdat)
rawdat$clf <- word(rawdat$nms, -1, sep = "_") # set to 2 for substrate group


# Subsmaple for 150 of the NRPS
nrps_sel <- sample(table(rawdat$clf)["NRPS"], 150, replace = F)
nrps_to_inc <- rawdat[rawdat$clf == "NRPS",][nrps_sel,]
dat <- data.frame(bind_rows(rawdat[rawdat$clf != "NRPS",], nrps_to_inc), stringsAsFactors = F)

# Remove the holdout test predictions
# dat <- dat[-grep(paste0(c("HOLDOUT", "OTHER", "CAR", "amino.acid"), collapse = "|"), dat$nms),] # 658 observations
...
pdf("data/model_comparisons/rf_feat_select_var_imp_funct_class_20191604.pdf", width = 6, height = 6)
ggplot(data = vimp, 
...
pllist[[1]]
ggsave("data/model_comparisons/aa34_rf_conf_matrices_funct_class_20191604.jpeg", height=4, width=5, units='in')

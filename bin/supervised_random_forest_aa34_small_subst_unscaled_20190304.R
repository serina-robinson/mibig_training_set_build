...
write_csv(data.frame(table(dat$clf)), "output/classification_results.csv")
# Set seed 
set.seed(20190304)
# write_delim(data.frame(colnames(df_train)), "~/Documents/Wageningen_UR/github/pallas/shiny_app/data/feature_names.txt")
# as.character(fread("~/Documents/Wageningen_UR/github/pallas/shiny_app/data/feature_names.txt", data.table = F)[,1])
...
# write_delim(data.frame(rownames(form_test)), "output/test_set_seqs_for_tool_prediction.csv")
...
saveRDS(rf, "data/model_comparisons/rf_xvalidated_small_subst_grp_unscaled_20190304.rds")
...
saveRDS(rf, "data/model_comparisons/rf_xvalidated_small_subst_grp_20190304.rds")
...
rf_ml <- ranger(y_train ~., data = form_train, num.trees = 500, splitrule = as.character(rf$bestTune$splitrule), 
                ...
                importance = "permutation")
saveRDS(rf_ml, "data/model_comparisons/rf_small_subst_group_unscaled_20190304.rds")
rf_ml <- readRDS("data/model_comparisons/rf_small_subst_group_unscaled_20190304.rds")

rf_pred <- predict(rf, newdata = form_test)
cm_rf <- confusionMatrix(rf_pred, as.factor(dat_test$clf))
cm_rf
sink("data/model_comparisons/cm_rf_15aa_functional_class_20190304.txt")
cm_rf
sink()
...
pdf("data/model_comparisons/rf_feat_select_var_imp_small_subst_group_unscaled_20190304.pdf", width = 6, height = 6)
ggplot(data = vimp, 
...
write.csv(dtl_feat_select, "data/model_comparisons/rf_AA34_Precision_Recall_Accuracy_Small_substrate_group_unscaled.csv", row.names = T)
...
pllist[[1]]
ggsave("data/model_comparisons/aa34_rf_conf_matrices_subst_grp_unscaled_20190304.jpeg", height=7, width=7, units='in')

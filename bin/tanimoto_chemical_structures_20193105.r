...
# Read in the updated training set
training1 <- read_excel("data/combined_adenylate_forming_training_set_for_db_20191404.xlsx")
 # write.csv(training1[training1$functional_class == "NRPS",], "output/nonNRPS_peptide_training_set.csv")


train_ss <- training1 %>%
  dplyr::filter(!grepl("NRPS|HOLDOUT|OTHER", functional_class)) 
 #  mutate(sub_clnd = gsub("_", " ", substrate)) 

...
rf_fc <- ranger(y_train ~., data = form_train, num.trees = 1000, splitrule = "gini",
                ...
                max.depth = 9, importance = "permutation", probability = TRUE) 


saveRDS(rf_fc, "data/rf_functional_class_1000trees_probability_centered_scaled.rds")


## Now try it with class weights
# n_samples / (n_classes * np.bincount(y))

classwts <- nrow(dat)/(length(unique(dat$clf)) * table(dat$clf))
rf_weighted_no_prob <- ranger(y_train ~., data = form_train, num.trees = 1000, splitrule = "gini",
                mtry = as.integer(sqrt(ncol(x_train))), min.node.size = 1,
                max.depth = 9, importance = "permutation", class.weights = classwts)
rf_fc_pred <- predict(rf_weighted_no_prob, data = form_test)

cm_rf_fc <- confusionMatrix(rf_fc_pred$predictions, as.factor(dat_test$clf))
cm_rf_fc$byClass

rf_unweighted_no_prob <- ranger(y_train ~., data = form_train, num.trees = 1000, splitrule = "gini",
                        mtry = as.integer(sqrt(ncol(x_train))), min.node.size = 1,
                        max.depth = 9, importance = "permutation")
rf_fc_pred <- predict(rf_unweighted_no_prob, data = form_test)

...
colnames(musc_df)[1] <- "nms"
musc_df$clf <- word(musc_df$nms, 2, sep = "_")

hmm_adom_df <- read_csv("output/20192305_HMMAlign_15aa_a_dom_properties.csv")
colnames(hmm_adom_df)[1] <- "nms"
hmm_adom_df$clf <- word(hmm_adom_df$nms, 2, sep = "_")

hmm_amp_df <- read_csv("output/20192305_HMMAlign_15aa_amp_binding_properties.csv")
colnames(hmm_amp_df)[1] <- "nms"
hmm_amp_df$clf <- word(hmm_amp_df$nms, 2, sep = "_")

# Create a list of data frames
df_list <- list(musc_df, hmm_adom_df, hmm_amp_df)

dat_split <- initial_split(musc_df, prop = 3/4, strata = "clf")
dat_split2 <- initial_split(hmm_adom_df, prop = 3/4, strata = "clf")
...
saveRDS(rf_results_list[[1]], "data/model_comparisons/rf_MUSCLE_aligned_ss_fulltrain_20193105_without_test.rds")
saveRDS(rf_results_list[[2]], "data/model_comparisons/rf_A_dom_aligned_ss_fulltrain_20193105_without_test.rds")
saveRDS(rf_results_list[[3]], "data/model_comparisons/rf_AMP_binding_aligned_ss_fulltrain_20193105_without_test.rds")
